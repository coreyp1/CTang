...
# hidden symbols.
LIB_CFLAGS := $(CFLAGS) -fvisibility=hidden -DG_TANG_BUILD
...
LIBVER_GEN := $(GEN_DIR)/tang/libver_gen.h
...
$(LIBVER_GEN): Makefile
	...
	@printf '%s\n' \
		...
		'#ifndef G_TANG_LIBVER_GEN_H' \
		'#define G_TANG_LIBVER_GEN_H' \
		'' \
		'/** The symbol namespace for this build, from the Makefile'"'"'s BRANCH. */' \
		'#define G_TANG_NAME $(LIBVER_SYMBOL)' \
		'' \
		'/** Human-readable version of this build. */' \
		'#define G_TANG_VERSION "$(MAJOR_VERSION).$(MINOR_VERSION)$(BRANCH)"' \
		'' \
		'#endif // G_TANG_LIBVER_GEN_H' > $@
...
$(APP_DIR)/tang$(EXE_EXTENSION): src/tang.c | $(APP_DIR)/$(TARGET)
...
$(APP_DIR)/testTangLanguageParse$(EXE_EXTENSION): test/test-tangLanguageParse.cpp | $(APP_DIR)/$(TARGET)
...
$(APP_DIR)/testTangLanguageExecuteSimple$(EXE_EXTENSION): test/test-tangLanguageExecuteSimple.cpp | $(APP_DIR)/$(TARGET)
...
$(APP_DIR)/testTangLanguageExecuteComplex$(EXE_EXTENSION): test/test-tangLanguageExecuteComplex.cpp | $(APP_DIR)/$(TARGET)
...
$(APP_DIR)/testTangLanguageLibrary$(EXE_EXTENSION): test/test-tangLanguageLibrary.cpp | $(APP_DIR)/$(TARGET)
...
$(APP_DIR)/testBinary$(EXE_EXTENSION): test/test-binary.cpp | $(APP_DIR)/$(TARGET)
	@printf "\n### Compiling Binary JIT functions Test ###\n"
	@mkdir -p $(@D)
	$(CXX) $(CXXFLAGS) $(INCLUDE) -MMD -MP -MF $(APP_DIR)/testBinary.d -o $@ $< $(LDFLAGS) $(TESTFLAGS) $(TANGLIBRARY)

$(APP_DIR)/test$(EXE_EXTENSION): test/test.cpp | $(APP_DIR)/$(TARGET)
	@printf "\n### Compiling Tang Test ###\n"
	@mkdir -p $(@D)
...
ifeq ($(OS_NAME), Linux)
	...
	if [ -n "$$leaked" ]; then \
		...
		printf "%s\n" "$$leaked" >&2; \
		printf "\nEach needs a '#define <name> G_TANG(<name>)' line in namespace.h, or\n" >&2; \
