 ...
 * @file
 *
 * libFuzzer harness for the tang script parser and simplifier.
 *
 * The contract being fuzzed: for any byte string at all, gta_tang_parse_script
...
int LLVMFuzzerTestOneInput(const uint8_t * data, size_t size) {
  ...
  GTA_Ast_Node * ast = gta_tang_parse_script(source);
  if (ast) {
